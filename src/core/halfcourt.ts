...
import type { CourtLines, DrawCourtResult, ShotchartSettings } from "./types";

export function drawCourt(
  shotchartSettings: ShotchartSettings,
  node: { current: SVGSVGElement | null },
): DrawCourtResult {
  ...
  const base = d3
    ...
    .attr(
      "viewBox",
      `0 0 ${shotchartSettings.courtWidth} ${shotchartSettings.visibleCourtLength()}`,
    )
    .append("g")
  ...
  } else if (shotchartSettings.leagueSettings.leagueId === "college") {
    base
      .append("rect")
  ...
  for (const mark of shotchartSettings.leagueSettings.keyMarks) {
    for (const n of [1, -1]) {
      base
        .append("line")
